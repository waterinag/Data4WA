...
## Download Monthly WaPOR L1 v3 AETI Data
...
## Download Annual WaPOR L1 v3 AETI Data
...
for year in range(firstyear, lastyear + 1):
    ...
    filename = f"WAPOR-3.L1-AETI-A.{year}.tif"
    # output_filename = f"wapor3_eta_a_{year}_{month:02d}.tif"
    output_filename = f"WAPOR-3-L1-AETI-A-{year}.tif"
    output_path = os.path.join(output_folder, output_filename)
    ...
    # Remote file URL via /vsicurl/
    url = f"https://gismgr.fao.org/DATA/WAPOR-3/MAPSET/L1-AETI-A/{filename}"
    vsicurl_url = f"/vsicurl/{url}"
    temp_clip = os.path.join(output_folder, f"temp_{output_filename}")
